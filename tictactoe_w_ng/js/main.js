...
myApp.controller("myController", function($scope){
	...
	$scope.players = [
		{name: "Joker", board: [], wins: 0, winner: 0},
		{name: "Batman", board: [], wins: 0, winner: 1}
	];
	...
	$scope.playerPicks = function(thisCell, num) {
	    ...
	    if ($scope.currentTurn == 1) {
	      ...
	      $scope.gameOver($scope.endgame, $scope.currentTurn);
	      //$scope.currentTurn = 0;
    ...
    $scope.gameOver = function(game, counter){
	    if(game == true){
			...
			$scope.currentTurn = $scope.players[counter].winner;
